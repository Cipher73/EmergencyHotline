...
public class Adminsignup extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        username = findViewById(R.id.Aname);
        password = findViewById(R.id.APassword);
        confirm_password = findViewById(R.id.Aconfirm_pass);
        idNum = findViewById(R.id.AidNum);
        register = findViewById(R.id.Aregister);
        ...
        mAuth.createUserWithEmailAndPassword(email,pass).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    User user=new User(name,pass,id,email,true);
                    FirebaseDatabase.getInstance().getReference("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if(task.isSuccessful()) {
                                Toast.makeText(Adminsignup.this, "user has been successfully registered", Toast.LENGTH_LONG).show();
                                startActivity(new Intent(Adminsignup.this,DataBaseView.class));
                            }
                            else{
